...
def make_fc_fullimg_dataset(tag, val_size):
    ...
    npy_to_bin.flatten_and_bin(imgs1, labels1, 'datasets/bins/train_and_label_{0}_batch_1.bin'.format(tag))
    npy_to_bin.flatten_and_bin(imgs2, labels2, 'datasets/bins/train_and_label_{0}_batch_2.bin'.format(tag))
    npy_to_bin.flatten_and_bin(imgs3, labels3, 'datasets/bins/train_and_label_{0}_batch_3.bin'.format(tag))
    npy_to_bin.flatten_and_bin(imgs4, labels4, 'datasets/bins/train_and_label_{0}_batch_4.bin'.format(tag))
    npy_to_bin.flatten_and_bin(imgs5, labels5, 'datasets/bins/train_and_label_{0}_batch_5.bin'.format(tag))
